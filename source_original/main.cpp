...
#include <thread>
#include <chess.hpp> //chess.hpp only used to parsefen and renderfen in uci cmd_position
using namespace chess;

//https://github.com/imor/polyglot/blob/master/polyglot.cpp#L460
//https://github.com/deoshreyas/Chess-Polyglot-Reader/tree/main
...
void cmd_go(std::istringstream &is, const Board &board, NumbFishProbe *nbfish )
{
    ...
    std::async(std::launch::async, &NumbFishProbe::get_move, nbfish, board.getFen(), moveTime/1000.);
...
//  char fen[]="7r/2k5/2b5/R7/8/8/2r3P1/3R2K1 w - - 0 1";
  char fen[]="2r3k1/4npr1/2q5/pp5Q/3p3p/PB1P3P/1PP2PPB/4R1K1 w - - 0 37";
   ...
   NumbFishProbe nbfish(true,false); // nnue and book
    ...
    nbfish.get_move(board.getFen(),5.0);
